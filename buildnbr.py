...
def get_args(testargs=None):
    parser = argparse.ArgumentParser(
        description='Generate new build number for docker image.',
    ...
    parser.add_argument('manifest_temp', type=argparse.FileType('r', encoding='utf8'),
                        help='Path of manifest of recent docker build. Must be in ./manifest/')
    parser.add_argument('manifest_scope', choices=['global', 'local'],
                        help='global or local (subdir in ./manifest/')
    parser.add_argument('build_number_file', type=argparse.FileType('w', encoding='utf8'),
