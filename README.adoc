...
* http_proxy, https_proxy and no_proxy set on the docker host
* these 3 env variables included in the Dockerfile as follows:

    #? ARG http_proxy
    #? ARG https_proxy
    #? ARG no_proxy

`build.sh` will create and use the temporary Dockerfile.proxy~ with the arguments enabled if a proxy is detected.